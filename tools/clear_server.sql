...
-- The CRITICAL thing to do here is keep the android super-user present.
...
-- create a temporary table to keep a list of users who are not being deleted
-- for information about the daemon user see https://wiki.openmrs.org/display/docs/Daemon+User
CREATE TEMPORARY TABLE keep_users SELECT * FROM users WHERE (system_id IN ("admin", "daemon")) OR username="android";
...
-- clear out the parent to allow deleting without foreign key constraint interfering
